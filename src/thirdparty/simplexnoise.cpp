...
//}
inline uint8_t SimplexNoise::hash(int32_t i) const {
...
float SimplexNoise::fractal(size_t octaves, float x) const {
	...
	for (size_t i = 0; i < octaves; i++) {
		output += (amplitude * this->noise(x * frequency));
		denom += amplitude;
...
SimplexNoise::SimplexNoise(unsigned int seed, float frequency, float amplitude, float lacunarity, float persistence) :
